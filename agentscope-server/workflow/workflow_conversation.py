...
from utils.agentstudio import init_agentstudio
...
alice = ReActAgent(
    name="Alice",
    sys_prompt="你是一个名为 Alice 的学生，爱好唱歌。",
    model=model,
    formatter=formatter,
)
...
bob = ReActAgent(
    name="Bob",
    sys_prompt="你是一个名为 Bob 的学生，爱好跳舞。",
    model=model,
    formatter=formatter,
)
...
charlie = ReActAgent(
    name="Charlie",
    sys_prompt="你是一个名为 Charlie 的学生，爱好打篮球。",
    model=model,
    formatter=formatter,
)
...
async def example_memory() -> None:
    """打印 Alice 的记忆。"""
    memory = await alice.memory.get_memory()
    print("Alice 的记忆：\n")
    for msg in memory:
